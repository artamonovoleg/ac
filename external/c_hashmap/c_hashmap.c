...
#define panic(_msg_)                                                           \
  do                                                                           \
  {                                                                            \
    AC_ERROR("[ hashmap ] : %s (%s:%d)", (_msg_), __FILE__, __LINE__);         \
  }                                                                            \
  while (false)
...
{
  return (struct bucket*)(void*)(((char*)map->buckets) +
                                 (map->bucketsz * index));
}
...
//-----------------------------------------------------------------------------
static uint64_t
MM86128(const void* key, const size_t len, uint64_t seed)
{
...
#define FMIX32(h)                                                              \
  ...
  const uint32_t* blocks = (const uint32_t*)(const void*)(data + nblocks * 16);
